...
class PterobotEnv(MujocoEnv, utils.EzPickle):
    # Based on ant-v4 env
    """
    ## Description
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.user_init_qpos = np.array([-0.062, 0.033, 0.24, 0.99, -0.011, 0.14, -0.029, 1.2, 0, -0.011, 1.0, 0.011, -0.8, 0.79, 0.98, 0.01, -0.72, 0.81, 0.086, 0, -0.5, 0.03, -0.0028, -0.47])
        self.user_init_qvel = np.array([-0.0016, 0, -0.0011, 0.0016, 0.0022, 0.0082, 0.13, 0, -0.0085, -0.014, 0.0052, 0.021, 0.021, -0.0055, 0.0035, 0.03, 0.036, 0.009, 0, 0.0056, 0.0086, -0.0026, 0.012])
        ...
        obs_shape = 45 #32 #45 # Torso xyzw in radians (orientation), Torso Z, dx, dy, dz, angvel(x, y, z), 17 * 2 = 34 -> 4 + 1 + 3 + 3 + 34 = 45
    ...
    def step(self, action):
        ...
        if self.print_counter % 1000 == 0:
            print(f'FWD: {forward_reward:.2f} | HLT: {healthy_reward:.2f} | VRT: {vertical_reward:.2f} | CTRL: {self.control_cost(action):.2f} | RWD: {rewards:.2f} | TOT: {reward:.2f}')
            # print(f' CTRL: {self.control_cost(action):.2f} | RWD: {rewards:.2f} | TOT: {reward:.2f}')
    ...
    def reset_model(self):
        ...
        qpos = self.user_init_qpos + self.np_random.uniform(
